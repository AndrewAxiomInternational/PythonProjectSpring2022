...
from directkeys import A, D, PressKey, ReleaseKey, W
...
def process_img(image):
    ...
    processed_img = cv2.Canny(yellow_and_white, 300, 400)

    vertices = np.array([[10,500],[10,300],[300,200],[500,200],[800,300],[800,500],], np.int32)

    processed_img = roi(processed_img, [vertices])

    lines = cv2.HoughLinesP(processed_img, rho=1, theta=np.pi / 180, threshold=60, minLineLength=15, maxLineGap=80)
    m1 = 0
    m2 = 0
    try:
...
while True:
    ...
    if m1 < 0 and m2 < 0:
        ...
        counterright = 0
        counterstraight += 1
    if counterleft>20 or counterright>20 or counterstraight>100:
        print("sleep!!!!!!!!!!!!!!!!!!!!!!!!!")
        time.sleep(10)
        counterleft = 0
        counterright = 0
