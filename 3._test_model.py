...
def main():
    ...
    window = get_window_location('Grand Theft Auto V')
    screen = cv2.resize(grab_screen(window), (WIDTH, HEIGHT))
    prev = screen
    ...
    while (True):
        ...
        if not paused:
            ...
            prediction = np.array(prediction) * np.array([1, 2, 2, 2, 1.8, 3, 4, 4, 1])
